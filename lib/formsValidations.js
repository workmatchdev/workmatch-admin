...
export const BlogEntranceSchemaUpdate = Yup.object().shape({
    title: Yup.string().required('Campo requerido'),
    url: Yup.string().required('Campo requerido').matches(
        /^[a-z]+(?:-[a-z]+)*$/,
        "El formato requeirdo es abc-def-ghi y solo minusculas"),
...
export const validationSchemaEmailConfiguration = Yup.object().shape({
    host: Yup.string()
        .required('El campo host es obligatorio'),
    port: Yup.number()
        .required('El campo port es obligatorio')
        .integer('El valor de port debe ser un número entero'),
