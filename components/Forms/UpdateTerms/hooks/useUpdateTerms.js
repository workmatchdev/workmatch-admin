...
const useUpdateLandingPage = ({
    ...
    useEffect(() => {
        const getRoles = async () => {
            try {
                ...
                setInitialValues({terminosYcondiciones: initVal.terminosYcondiciones})
                setSubmitError(null)
                setisLoading(false)
            } catch (error) {
    ...
    const handleSubmit = async (values) => {
        try {
            const response = await axios.put('/api/pageBuilder/pages/updatePageTerms/65953ecc0969c10351a6e7ea', values);
